...
fl <- here::here("output", "qc", "raw", glue("{samples$accession}_fastqc.zip")) %>% 
...
Most data was obtained from [FastQC]() and parsed using the Bioconductor package ngsReports [@Ward2020-fn].
FastQC reports for `r length(rawFqc)` files were found.

A conventional MultiQC report [@Ewels2016-ix] can also be found [here](../output/qc/raw/multiqc.html)
      ...
      columns = list(
        Filename = colDef(
          cell = function(value) htmltools::tags$a(
            href = file.path(
              "..", "output", "qc", "raw", glue("{value}_fastqc.html")
            ), 
            target = "_blank", 
            value
          ),
...
plotReadTotals(rawFqc, vertBars = FALSE, divBy = 1e6) + myTheme
```
